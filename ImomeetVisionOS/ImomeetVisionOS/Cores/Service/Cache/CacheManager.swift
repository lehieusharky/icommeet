...
final class CacheManagers<Key: Hashable, Value : Any> {
    ...
    init( countLimit : Int = 100, expireTimes: TimeInterval = TimeInterval(3600 * timeout), dateProvider : @escaping () -> Date? = Date.init) {
    ...
    fileprivate func createCacheDirectory() -> Bool {
        ...
        let component = path.appendingPathComponent("data.cache")
